...
for(const name of ['ペロ','Ruffians','いたずらなRuffians'])test('0836 required search '+name,()=>{
 const c=cards.find(c=>c.number==='hBP08-036'),prior=cards.find(x=>x.jpName===c.jpName&&x.stage==='Debut');
 ...
 assert.equal(s.players[0].hand.at(-1).id,'pick');
});
