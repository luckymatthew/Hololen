...
def patch(root):
    ...
    outputs[runtime]=rt
    report={'alreadyApplied':False,'integrationVersion':'hbp09-integration-20260918.2','originalEngineSha256':hashlib.sha256(original.encode()).hexdigest(),'renamedLegacyFunctions':rename,'changedFiles':[str(p.relative_to(root)) for p in outputs]}
