"""Merge the pinned official hBP09 set without replacing unrelated cards.
Requires beautifulsoup4 for source conversion; downloaded code is never executed.
"""
...
def fields(row):
    ...
    for field in row['fields']:
        soup=BeautifulSoup(field['html'],'html.parser');div=soup.find('div');paragraphs=div.find_all('p',recursive=False)
        if len(paragraphs)!=2:raise ValueError(f'Unexpected field format: {row["number"]}')
        body=paragraphs[1];heading=body.find('span',recursive=False);classes=field['class'].split()
        if 'extra' in classes:
        ...
        special=heading.select_one('.tokkou');special_icons=[dict(img.attrs) for img in special.find_all('img')] if special else []
        if special:special.extract()
        title=clean(heading.get_text());heading.extract();effect=clean(body.get_text())
...
def build_release(source,names,existing):
    ...
    cards=[];japanese={};base_rarities=['OSR','RR','R','U','C','S','SY','SR','UR','OUR','HR','SEC']
    for card_number,printings in sorted(grouped.items()):
        ...
        group,typecode,label=TYPES[kind];colors=[]
        ...
        japanese[card_number]={'titles':titles,'effects':[text for text in effects if text]}
        cards.append(card)
...
def merge(base,release,local_images=None):
    ...
    for item in release['cards']:
        incoming=copy.deepcopy(item);n=incoming['number'];old=current.get(n)
        ...
        for variant in incoming['variants']:
            if variant['image'] in images:
                prior=images[variant['image']];prior['sets']=list(dict.fromkeys(prior.get('sets',[])+[SET]))
            else:
