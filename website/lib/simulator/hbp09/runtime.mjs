...
export const VERSION = 'hbp09-integration-20260918.2';
...
export function createRuntime(host) {
  ...
  function refs(state, context, selector = {}, map) {
    ...
    if (selector.area === 'stage') {
      result=SLOTS.flatMap(zone => {
        const unit=p.zones[zone]; if (!unit || unit.downPending) return [];
  ...
  function prompt(state,c,op,remaining,map) {
    ...
    if(kind==='chooseCards') {
     ...
     } else if(kind==='chooseZone') {
      const zones=(op.zones||[]).filter(zone=>SLOTS.includes(zone)&&zone.startsWith('back')&&!state.players[owner].zones[zone]);
      if(!zones.length){c.vars[op.key]=null;return false;}
      const card=c.vars.deployCard?.[0];
      state.pendingChoice={type:'stageTarget',playerIndex:owner,targetPlayerIndex:owner,options:zones,cardId:card?.id,cardNumber:card?.number,optional:false,prompt:op.prompt||`${c.sourceNumber}：選擇後排空位放置卡片`,effect:'hbp09',meta:{hbp09:{context:ctx,op:copy(op),remaining:copy(remaining)}}};
  ...
  function run(state,ctx,input,map,random) {
    ...
    while(steps.length && state.status!=='finished') {
      ...
      if(['chooseCards','chooseUnit','chooseZone','chooseOption'].includes(o.op)){
      ...
      if(o.op==='deploy'){
        ...
        steps.unshift({op:'chooseZone',key:'deployZone',zones,then:[{op:'placeSelected'},{op:'deploy',cards:{ref:'deployRemaining'}}]});continue;
      }
  ...
  function resolve(state,i,action,map,random) {
    ...
    if(pending.type==='cardSelection'){
      ...
      if(o.op==='chooseZone'){
        fail(pending.options.includes(zone)&&!state.players[pending.targetPlayerIndex].zones[zone],'Invalid/occupied stage destination');result=zone;
      }else{result=skipped?[]:data.candidates.filter(r=>r.zone===zone);fail(skipped||(result.length===1&&locate(state,result[0])),'Invalid/stale stage target');}
