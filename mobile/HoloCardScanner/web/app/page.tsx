      ...
      >
        ...
        <div className="modal-copy">
          ...
          <div className="effect-stack">
            ...
            <TerminologyNote />
            {effectAuditPending[card.number] && <p role="note">校對狀態：{effectAuditPending[card.number]}</p>}
...
export default function Home() {
  ...
  const [workspace, setWorkspaceState] = useState<"library" | "deck">(location.hash === "#deck" ? "deck" : "library");
  const setWorkspace = (value: "library" | "deck") => { setWorkspaceState(value); location.hash = value; };
  ...
  useEffect(() => {
    const onHash = () => { if (["#library", "#deck"].includes(location.hash)) setWorkspaceState(location.hash === "#deck" ? "deck" : "library"); };
    const onExport = (event: Event) => setNotice((event as CustomEvent).detail.ok ? "已儲存匯出檔案。" : "未有儲存檔案；牌組不受影響。");
    window.addEventListener("hashchange", onHash); window.addEventListener("native-export", onExport);
    return () => { window.removeEventListener("hashchange", onHash); window.removeEventListener("native-export", onExport); };
  }, []);
  ...
  useEffect(() => {
    Promise.all([
      ...
      fetch("/holosim-card-index.json").then((response) => response.ok ? response.json() as Promise<HoloSimCardIndex> : {}),
    ])
  ...
  useEffect(() => {
    try {
      ...
      setDeckName(localStorage.getItem("hololive-ocg-draft-name") || "未命名牌組");
      const localId = new URLSearchParams(location.search).get("localDeck");
      if (localId) {
        const local = JSON.parse(localStorage.getItem("hololive-ocg-local-decks") || "[]").find((d: {id: string}) => d.id === localId);
        if (local) { setDeck(local.deck); setDeckName(local.name); setWorkspace("deck"); }
      }
    } catch {
      setDeck(emptyDeck());
    } finally {
  ...
  useEffect(() => {
    if (deckReady) {
      try { localStorage.setItem(storageKey, JSON.stringify(deck)); localStorage.setItem("hololive-ocg-draft-name", deckName); }
      catch { setNotice("裝置儲存空間不足，請匯出備份牌組。"); }
    }
  }, [deck, deckReady, deckName]);
  ...
  useEffect(() => {
    ...
    if (!id) return;
    setWorkspace("deck");
    fetch(`/api/decks/${encodeURIComponent(id)}`, {signal: AbortSignal.timeout(15000)})
      .then(async (response) => {
  ...
  const saveDeck = async () => {
    ...
    setSaving(true);
    try {
      const response = await fetch(activeDeckId ? `/api/decks/${encodeURIComponent(activeDeckId)}` : "/api/decks", {
        method: activeDeckId ? "PUT" : "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ name: proposedName, deck }),
      ...
      setActiveDeckId(payload.deck.id);
      setDeckName(payload.deck.name);
      window.history.replaceState({}, "", `/?deck=${encodeURIComponent(payload.deck.id)}#deck`);
      setNotice(`「${payload.deck.name}」已保存到你的帳號。`);
    } catch (error) {
  ...
  const saveLocalDeck = () => {
    if (!deckName.trim() || !oshiCount && !mainCount && !cheerCount) { setNotice("請先加入卡片及輸入牌組名稱。"); return; }
    try {
      const rows = JSON.parse(localStorage.getItem("hololive-ocg-local-decks") || "[]");
      const currentId = new URLSearchParams(location.search).get("localDeck");
      const id = rows.some((row: {id: string}) => row.id === currentId) ? currentId : crypto.randomUUID();
      if (rows.length >= 50 && !rows.some((row: {id: string}) => row.id === id)) { setNotice("已達 50 副本機牌組上限，請先備份或刪除不需要的牌組。"); return; }
      const next = {id, name: deckName.trim(), deck, updatedAt: Date.now()};
      localStorage.setItem("hololive-ocg-local-decks", JSON.stringify([next, ...rows.filter((row: {id: string}) => row.id !== id)]));
      history.replaceState({}, "", `/?localDeck=${encodeURIComponent(id!)}#deck`);
      setNotice("已儲存到本機牌組庫。");
    } catch { setNotice("本機儲存失敗，請匯出檔案備份。"); }
  };
  ...
  const exportDeck = () => {
    ...
    if (window.HoloNative) {
      window.HoloNative.exportJson(toHoloSimFilename(proposedName), JSON.stringify(result.deck, null, 2));
      return;
    }
  ...
  const exportBackup = () => {
    const name = deckName.trim() || "未命名牌組";
    window.HoloNative?.exportJson(`${name}-完整備份.json`, JSON.stringify({format:"holo-pocket-deck",version:1,name,deck}, null, 2));
  };
    ...
    try {
      ...
      for (const section of ["oshi", "main", "cheer"] as DeckSection[]) {
        for (const [number, rawCount] of Object.entries(incoming[section] as Record<string, number>)) {
          ...
          const count = Math.max(0, Math.min(card.maxCopies, Math.floor(Number(rawCount) || 0)));
          if (count > 0) next[cardSection(card)][number] = count;
        }
      }
      if (incoming.printings && typeof incoming.printings === "object") {
        ...
        for (const [number, rawAllocation] of Object.entries(incoming.printings as PrintingState)) {
          ...
          for (const [variantId, rawVariantCount] of Object.entries(rawAllocation)) {
            const variantCount = Math.max(0, Math.min(total - allocated, Math.floor(Number(rawVariantCount) || 0)));
      ...
      setDeck(next);
      setActiveDeckId(null);
      setDeckName(typeof parsed.name === "string" && parsed.name.trim() ? parsed.name.trim().slice(0,60) : file.name.replace(/\.json$/i, "") || "匯入牌組");
      window.history.replaceState({}, "", "/#deck");
      setWorkspace("deck");
  ...
  useEffect(() => {
    const imported = (event: Event) => {
      ...
      void importDeck(new File([data.json], data.name, {type:"application/json"}));
    };
    window.addEventListener("native-import", imported);
    return () => window.removeEventListener("native-import", imported);
  }, [cardMap]);
  ...
  return (
    <main className="studio-page" id="top">
      ...
      <div className="studio-masthead">
        <div>
          <p className="eyebrow">HOLO · POCKET LAB</p>
          <h1>{workspace === "library" ? "卡片圖鑑" : "牌組工房"}</h1>
          <p>{workspace === "library" ? "繁中效果，一掃即查。" : "選卡圖、調配比，隨時儲存。"}</p>
        </div>
      ...
      <section className="deck-workbench" id="deck-workbench" aria-label="牌組構築器" hidden={workspace !== "deck"} tabIndex={-1}>
        ...
        <div className="deck-actions">
          <button className="save-button" type="button" onClick={saveLocalDeck}>儲存到本機</button>
          <button className="save-button" type="button" onClick={() => void saveDeck()} disabled={saving}>{saving ? "保存中…" : activeDeckId ? "更新保存" : "雲端儲存"}</button>
          <button className="export-button" type="button" onClick={exportDeck}>匯出 HoloSim 1.13 JSON</button>
          <button className="export-button" type="button" onClick={exportBackup}>匯出完整備份</button>
          <button className="import-button" type="button" onClick={() => window.HoloNative?.importJson()}>匯入 JSON</button>
