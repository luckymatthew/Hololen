...
export default function FirebaseAccount() {
  ...
  return <main className="account-page">
    ...
    <div className="saved-deck-grid">{decks.map(deck => <article className="saved-deck-panel" key={deck.id}><div className="saved-deck-head"><div><p>{deck.archived ? '已封存 · 可隨時還原' : `UPDATED ${new Date(deck.updatedAt).toLocaleDateString('zh-Hant')}`}</p><h2>{deck.name}</h2></div></div><DeckPreview deck={deck} cardMap={cardMap} /><div className="saved-deck-actions"><a href={`/?deck=${encodeURIComponent(deck.id)}`}>載入組牌器</a><button onClick={() => archiveDeck(deck.id, !deck.archived)}>{deck.archived ? '還原牌組' : '封存'}</button></div></article>)}</div>
    {!decks.length && <div className="no-saved-decks"><h2>還沒有保存的牌組</h2><p>在組牌器按「保存牌組」，未登入也能保存。</p></div>}
