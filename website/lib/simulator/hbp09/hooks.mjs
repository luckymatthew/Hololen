...
export function createHbp09(host) {
  ...
  function customOp(state,c,o,steps,map,random) {
    ...
    if(o.op==='resolveEndTrigger'){
      ...
      const [effect]=triggers.splice(index,1);
      rt.enqueue(state,effect.context,effect.steps);
      if(triggers.length)rt.enqueue(state,c,[{op:'orderEndTriggers'}]);
      return true;
    }
  ...
  function endPerformance(state,i,map){
    ...
    p.hbp09EndProcessedTurn=state.turn;const triggers=[];
    const add=(num,zone,steps)=>{triggers.push({context:rt.context(state,i,num,zone,{event:'performanceEnd'}),steps});};
    ...
    if(triggers.length){
      ...
      host.enqueueEffect(state,{type:'hbp09FinishTurn',playerIndex:i});
