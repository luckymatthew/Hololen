...
for(const assistants of [0,2])test('Koyori 024 actual Cheer reveal '+assistants,()=>{
 ...
 s=applyAction(s,0,{type:'choose',allocations:{center:3}},pool,()=>0);
 assert.equal(s.players[0].zones.center.damage,40);assert.equal(s.players[1].zones.center.damage,180);assert.equal(s.pendingChoice,null);assert.equal(s.players[0].mainDeck.length,7);
});
