...
test('Subaru search copies selected enemy Bloom level and deploys without losing cards',()=>{
 ...
 out=answer(out,{optionId:'back1'});assert.equal(out.players[0].zones.back1.stack[0].number,N(10));conserve(s,out);
});
...
test('Hajime passive reduces actual received Arts damage by 30',()=>{const s=fixture(2,17);s.activePlayer=1;s.turn=4;s.phase='performance';const attacker={...dummy,number:'TEST-ATTACKER',arts:[{name:'Hit',damage:60,cost:[],effect:''}]};const out=act(s,{type:'attack',sourceZone:'center',targetZone:'center',artIndex:0},1,[...cards,attacker]);assert.equal(out.players[0].zones.center.damage,30);});
