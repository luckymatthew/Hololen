...
function DeckPreview({ deck, cardMap }: { deck: SavedDeck; cardMap: Map<string, Card> }) {
  return (
    <div className="saved-deck-sections">
      {(["oshi", "main", "cheer"] as const).map((section) => {
        const rows = Object.entries(deck.deck[section]).flatMap(([number, total]) => {
          ...
          let allocated = 0;
          const printingRows: {number:string;count:number;card:Card|undefined;variant:NonNullable<Card["variants"]>[number]|undefined}[] = Object.entries(allocation).flatMap(([variantId, rawCount]) => {
            const variant = variants.find((candidate) => candidate.id === variantId);
            const count = Math.max(0, Math.min(total - allocated, Number(rawCount) || 0));
            if (!variant || count <= 0) return [];
            allocated += count;
            return [{ number, count, card, variant }];
          });
...
export default function AccountClient() {
  ...
  const loadDecks = async () => {
    setDeckLoading(true);
    try {
      const response = await fetch("/api/decks", {signal: AbortSignal.timeout(15000)});
      if (!response.ok) throw new Error("無法讀取雲端牌組，請檢查連線後再試。");
      setDecks((await response.json()).decks);
    } catch { setAuthError("暫時無法讀取雲端牌組，本機草稿不受影響。"); }
    finally { setDeckLoading(false); }
  };

  useEffect(() => {
    Promise.all([
      fetch("/api/auth/session", {signal: AbortSignal.timeout(12000)}).then((response) => response.json()),
      fetch("/cards.json").then((response) => response.json() as Promise<CardPayload>),
    ]).then(([session, payload]) => {
      setUser(session.user);
      setCards(payload.cards);
      setChecking(false);
      if (session.user) void loadDecks();
    }).catch(() => {setChecking(false);setAuthError("帳號服務暫時未能連接；你仍可使用本機牌組及卡庫。");});
  }, []);
  ...
  const submitAuth = async (event: FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) setAuthError(payload.error || "帳號服務暫時無法使用，請稍後再試。");
        ...
        if (returnTo) { const destination = new URL(returnTo, location.origin); if (destination.origin === location.origin && destination.pathname === "/") location.href = destination.pathname + destination.search + "#deck"; }
      }
    } catch (error) {
  ...
  const logout = async () => {
    try {const response=await fetch("/api/auth/logout", { method: "POST", signal:AbortSignal.timeout(12000) });if(!response.ok)throw new Error();setUser(null);setDecks([]);}
    catch {setAuthError("登出未完成，請檢查網絡後再試。");}
  };

  const deleteDeck = async (deck: SavedDeck) => {
    if (!window.confirm(`確定刪除「${deck.name}」嗎？`)) return;
    try {const response = await fetch(`/api/decks/${encodeURIComponent(deck.id)}`, { method: "DELETE",signal:AbortSignal.timeout(12000) });
    if (response.ok) setDecks((current) => current.filter((item) => item.id !== deck.id));else throw new Error();}
    catch {setAuthError("未能刪除雲端牌組，請稍後再試。");}
  };
    ...
    <main className="account-page">
      ...
      {checking ? (
        ...
        <section className="deck-library-page">
          {authError && <p className="auth-error" role="alert">{authError}<button type="button" onClick={() => {setAuthError("");void loadDecks();}}>重新讀取</button></p>}
